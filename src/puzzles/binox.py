...
class Binox(Model):
    ...
    def addVariables(self) -> None:
        ...
        self.yVars = {}
        for col in range(self.data['shape']):
            for row1, row2 in itertools.combinations(range(self.data['shape']), 2):
                self.yVars[(row1, row2, col)] = self.addVariable(
                    vtype=mip.BINARY, name=f'y_{row1}_{row2}_{col}'
                )
        self.zVars = {}
        for row in range(self.data['shape']):
            for col1, col2 in itertools.combinations(range(self.data['shape']), 2):
    ...
    def addNoMoreThanNSameSymbolTouchingInRowConstraints(self) -> None:
        for row in range(self.data['shape']):
            for col in range(self.data['shape'] - self.data['nSymbols']):
                self.addConstraint(
    ...
    def addNoMoreThanNSameSymbolTouchingInColConstraints(self) -> None:
        for col in range(self.data['shape']):
            for row in range(self.data['shape'] - self.data['nSymbols']):
                self.addConstraint(
    ...
    def addEqualNumberOfXsAndOsInEachRowConstraints(self) -> None:
        for row in range(self.data['shape']):
            self.addConstraint(
                mip.xsum(self.xVars[row][col] for col in range(self.data['shape'])) == self.data['shape'] / 2
            )
        return None
    ...
    def addEqualNumberOfXsAndOsInEachColConstraints(self) -> None:
        for col in range(self.data['shape']):
            self.addConstraint(
                mip.xsum(self.xVars[row][col] for row in range(self.data['shape'])) == self.data['shape'] / 2
            )
        return None
    ...
    def addUniqueRowConstraints(self) -> None:
        ...
        for row1, row2 in itertools.combinations(range(self.data['shape']), 2):
            self.addConstraint(
                mip.xsum(self.yVars[(row1, row2, col)] for col in range(self.data['shape'])) <= self.data['shape'] - 1
            )
        return None
    ...
    def addUniqueColConstraints(self) -> None:
        ...
        for col1, col2 in itertools.combinations(range(self.data['shape']), 2):
            self.addConstraint(
                mip.xsum(self.zVars[(col1, col2, row)] for row in range(self.data['shape'])) <= self.data['shape'] - 1
            )
        return None
